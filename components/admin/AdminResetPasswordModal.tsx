  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/admin/change-password", {
        ...
        body: JSON.stringify({ authorId, newPassword }),
      });
  ...
  return (
    <>
      <button
        type="button"
        onClick={handleOpen}
        className="inline-flex items-center gap-1 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 px-2.5 py-1 rounded transition-colors"
        title={`Set or change password for ${authorName}`}
      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-sm bg-white rounded-lg shadow-2xl p-6 relative">
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                <h2 className="font-serif font-bold text-base text-slate-900">Set Author Password</h2>
                <p className="text-xs text-slate-500">
                  Author: <span className="font-semibold text-slate-800">{authorName}</span>
                </p>
              </div>
            </div>
            ...
            {success ? (
              <div className="py-6 flex flex-col items-center justify-center text-center gap-2">
                ...
                <p className="text-xs text-slate-500">The author can now log in with this new password.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-3.5">
                <p className="text-xs text-slate-600 bg-slate-50 border border-slate-200 rounded p-2.5">
                  As an administrator, you can directly set or override this author&apos;s password without entering their current one.
                </p>
